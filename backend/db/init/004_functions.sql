...
-- =====================================================
-- FUNCIÓN: validar_equipo_unico_en_torneo_mismo_club
-- Propósito: un club con varios equipos (p.ej. Primera y Reserva) inscriptos
-- en el mismo torneo no puede anotar a la misma persona en dos planteles de
-- ese torneo. `validar_rol_unico_por_club` no lo detecta porque solo mira
-- clubes distintos. Ver migración 0036.
-- =====================================================
...
-- Propósito: Función combinada para validar rol en plantel, rol único por
-- club y equipo único por torneo dentro del mismo club
-- =====================================================
...
BEGIN
    ...
    -- 2. Validar regla de rol único por club (club distinto)
...
-- Regla 1 (exceptuable): la persona tiene un rol activo (cualquiera)
-- en el club local o visitante del partido.
...
-- Regla 2 (absoluta): la persona integra un plantel de un equipo
-- inscripto en el mismo torneo del partido.
