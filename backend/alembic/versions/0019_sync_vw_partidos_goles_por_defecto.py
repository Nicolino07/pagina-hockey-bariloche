"""Suma goles_por_defecto en los totales de vw_partidos_detallados

Alinea la vista con la definición de los scripts de init: los totales
goles_local / goles_visitante ahora incluyen los goles otorgados por
defecto (walkover / partido perdido administrativamente).
...
def downgrade() -> None:
    # Vuelve a la vista de la 0017 (sin sumar goles_por_defecto en los totales)
    op.execute("DROP VIEW IF EXISTS vw_partidos_detallados CASCADE")
    op.execute("""
    CREATE VIEW vw_partidos_detallados AS
     SELECT p.id_partido,
