"""Actualiza vw_partidos_detallados para incluir capitán en lista de jugadores

Agrega el campo capitan (true/false) al final de cada entrada en
lista_jugadores_local y lista_jugadores_visitante.
Formato anterior: apellido|nombre|camiseta|rol
Formato nuevo:    apellido|nombre|camiseta|rol|capitan
 ...
 SELECT p.id_partido,
    ...
    COALESCE(p.goles_visitante_manual::bigint, ( SELECT count(*) AS count
           FROM gol g
             ...
             JOIN plantel pl ON pi.id_plantel = pl.id_plantel
          WHERE g.id_partido = p.id_partido AND (pl.id_equipo = itv.id_equipo AND NOT g.es_autogol OR pl.id_equipo = itl.id_equipo AND g.es_autogol))) AS goles_visitante,
    ( SELECT string_agg((((((per.apellido::text || '|'::text) || per.nombre::text) || '|'::text) || COALESCE(pp.numero_camiseta::text, ''::text)) || '|'::text) || pi.rol_en_plantel, '; '::text ORDER BY pi.rol_en_plantel DESC, pp.numero_camiseta) AS string_agg
           FROM participan_partido pp
             JOIN plantel_integrante pi ON pp.id_plantel_integrante = pi.id_plantel_integrante
             JOIN persona per ON pi.id_persona = per.id_persona
             JOIN plantel pl ON pi.id_plantel = pl.id_plantel
